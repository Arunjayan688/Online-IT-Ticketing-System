    ...
    public class DatabaseHelper
    {
        ...
        public static int GetSubAdminIdByUsername(string username)
        {
            ...
            try
            {
                using (var connection = new SqlConnection (new DatabaseHelper().GetConnectionString()))
                {
                    string query = "SELECT Id FROM SubAdmins WHERE Username = @Username";
        ...
        public static List<TicketModel> GetTicketsForSubAdmin(int subAdminId)
        {
            ...
            try
            {
                using (var connection = new SqlConnection(new DatabaseHelper().GetConnectionString()))
                {
                    string query = "SELECT TicketId, Username, Subject, Category, CreationDate, Status, AttachmentPath, AttachmentData FROM Tickets WHERE AssignedTo = @SubAdminId ORDER BY CreationDate DESC";
                    using (var command = new SqlCommand(query, connection))
                    {
